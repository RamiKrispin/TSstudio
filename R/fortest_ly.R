fortest_ly <- function(actual, forecast.obj, train, test, Ygrid = FALSE, Xgrid = FALSE){
  # Error handling
  ...
  plotly::add_trace(x = time_actual, 
            y = c(forecast.obj$fitted, rep(NA,length(actual) - length(train))), 
            mode = 'lines+markers',name = "Fitted", type = 'scatter',
            line = list(color = "red")) %>%
  plotly::add_trace(x = time_actual, 
            y = c(rep(NA,length(actual) - length(test)), forecast.obj$mean), 
            mode = 'lines+markers',name = "Forecast",type = 'scatter') %>% 
  plotly::layout(
    title = "Actual vs Forecast and Fitted", 
