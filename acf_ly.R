acf_ly <- function(ts.obj, lag.max = NULL, ci = 0.95){
  ...
  } else if(!stats::is.ts(ts.obj) & !xts::is.xts(ts.obj) & !zoo::is.zoo(ts.obj)){
  ...
  df <- data.frame(lag = base::round(x[[4]], 3), 
                   acf = base::round(x[[1]], 3), 
                   width = 0.02, 
                   ci_u = base::round(ci_value,2), 
        ...
        for(c in series){
          ...
          if(r == c){
            ...
            series_list[counter] <- plotly::plot_ly() %>% 
              ...
              plotly::add_trace(x = df[, counter], 
                                y = df$ci_d, 
                                type = "scatter", mode = "lines", name = "CI Lower Bound",   
                                line = list(width = 1, dash = "dash", color = "green")) %>%
              plotly::layout(
                xaxis = list(title = "Lag", showgrid = FALSE),
                yaxis = list(title = "ACF", showgrid = FALSE, range = c(min(x[[1]]), max(x[[1]]))),
